 ...
 * 
 * Usage:
 *   npx tsx scripts/benchmark/sprint2-bench.ts
 *   npx tsx scripts/benchmark/sprint2-bench.ts --files=500 --out=custom.json
 */
...
try {
  ...
  const parseSingle = bench("parse single file", () => {
    runTsScript(PARSE_SCRIPT, [files[0]])
  })
  ...
  for (const f of files) {
    const r = runTsScript(PARSE_SCRIPT, [f], { timeout: 5000 })
    if (r.status === 0) {
  ...
  const shakeResult = bench("shake CSS", () => {
    ...
    runTsScript(SHAKE_SCRIPT, [copy, "--classes-from", shakeTestSrc])
    fs.unlinkSync(copy)
  })
  ...
  const shakeRun = runTsScript(SHAKE_SCRIPT, [shakeCopy, "--classes-from", shakeTestSrc])
  let shakeStats = {}
  ...
  const clusterInit = runTsScript(CLUSTER_SCRIPT, ["init", String(Math.min(4, os.cpus().length))], {
    encoding: "utf8", cwd: tmpDir
  })

  const clusterBuild = bench("cluster build", () => {
    runTsScript(CLUSTER_SCRIPT, ["build", srcDir], { cwd: tmpDir, timeout: 30_000 })
  }, 2)

  let clusterStats = {}
  const clusterFinal = runTsScript(CLUSTER_SCRIPT, ["build", srcDir], { cwd: tmpDir })
